---
title: "MBD4 loss-of-function is associated with a replication stress-linked ATR inhibitor vulnerability and does not support a transcriptional PARP1-upregulation model for PARP-trapping sensitivity: a pharmacogenomic hypothesis-generation study"
...
abstract: |
 ...
 First, MBD4-LOF cancer cell lines show significantly enhanced sensitivity to the ATR inhibitor
 ceralasertib (AZD6738) compared to wild-type lines (LN_IC50 Δ=−0.73, p=0.021, Cohen's d=−0.50;
 n=14 True-LOF vs 914 WT; GDSC2), a finding that survives four orthogonal confound stress tests
 including MSI-H purge (p=0.015), TP53 stratification (p=0.003), leave-one-out robustness (14/14
 iterations), and lineage-matched comparison; MSI-H collinearity remains a major structural limitation
 of the observational design. Second, MBD4-LOF lines do not exhibit significantly elevated PARP1
...
One mechanistic model that follows from this connection is that MBD4 loss creates constitutive replication stress through accumulation of unprocessed U:G mismatches at CpG dinucleotides. Under this model, during S-phase these lesions are encountered by the replication machinery, generating single-stranded DNA (ssDNA) gaps and stalled forks. Exposed ssDNA would recruit RPA, which in turn recruits the ATR-ATRIP complex via TOPBP1, activating CHK1 and the intra-S and G2/M checkpoints. If correct, BER-deficient cells would be constitutively dependent on ATR checkpoint signaling to tolerate the ongoing replication stress imposed by unrepaired base lesions — a dependency that ATR inhibitors would be expected to exploit. This mechanistic chain is distinct from the PARP-trapping model, which requires elevated PARP1 substrate availability rather than checkpoint dependency. The present study does not directly test this chain at the molecular level; rather, it uses pharmacogenomic stratification to ask whether the predicted ATR dependency is detectable in publicly available cell line drug sensitivity data.
...
Here we address both questions using pharmacogenomic stratification of publicly available cell line drug sensitivity data. We report a pharmacogenomic association between MBD4-LOF and ATR inhibitor sensitivity validated through four orthogonal confound stress tests, falsify the hypothesis that PARP1 is transcriptionally upregulated as an adaptive stress biomarker, and generate the testable hypothesis that MBD4-LOF creates an ATR checkpoint dependency warranting isogenic validation.
