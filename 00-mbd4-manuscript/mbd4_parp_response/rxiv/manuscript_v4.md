---
title: "MBD4 loss-of-function is associated with ATR inhibitor sensitivity and does not support a direct transcriptional PARP1-upregulation model: a pharmacogenomic hypothesis-generation study"
...
abstract: |
 ...
 generate the hypothesis that MBD4-LOF may be associated with a replication stress-linked ATR
 checkpoint dependency that is mechanistically distinct from PARP trapping. Isogenic validation — specifically ATRi
...
One mechanistic model that follows from this connection proposes that MBD4 loss generates constitutive replication stress through accumulation of unprocessed U:G mismatches at CpG dinucleotides. Under this model, these lesions stall replication forks during S-phase, activating the ATR-CHK1 checkpoint axis and creating a dependency on ATR kinase activity for cell viability — a dependency that ATR inhibitors would be predicted to exploit. This proposed model is distinct from a PARP-trapping model, which depends on elevated PARP1 substrate availability rather than checkpoint reliance. The present study does not directly test this chain at the molecular level; rather, it uses pharmacogenomic stratification to ask whether the predicted ATR dependency is detectable in publicly available cell line drug sensitivity data.
...
One mechanistic model proposes that MBD4 loss generates constitutive replication stress through accumulation of unprocessed base lesions at CpG sites, activating the ATR-CHK1 checkpoint axis and creating a dependency on ATR kinase activity for cell viability. We tested whether this predicted association is detectable in pan-cancer pharmacogenomic data.
...
These pharmacogenomic observations generate a testable hypothesis: MBD4-LOF cells may be constitutively dependent on ATR checkpoint signaling in a manner exploitable by ATR inhibitors. Definitive validation requires isogenic ATRi sensitivity testing in MBD4-knockout vs. MBD4-rescue cell lines, which is the critical next experiment.
