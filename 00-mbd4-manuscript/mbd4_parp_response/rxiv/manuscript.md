...
![**MBD4-LOF defines a pan-cancer ceralasertib-sensitive state.** GDSC2 LN_IC50 comparison between MBD4 True-LOF lines (n=14) and the canonical MBD4-WT pool (n=914). MBD4-LOF shifts LN_IC50 by −0.732 (one-sided Mann–Whitney p=0.0215; Cohen's d=−0.503), corresponding to a 2.08-fold lower geometric-mean IC50.](FIGURES/fig2_ceralasertib_volcano.png){#fig:volcano}

![**Orthogonal validation resolves the strongest ceralasertib response states.** MSI-H exclusion sharpens the MBD4-LOF effect (n=10 vs 906; Δ LN_IC50=−0.910; p=0.0153; d=−0.623). Within TP53-mutant models, the response reaches Δ LN_IC50=−1.069 (n=11 vs 619; p=0.0030; d=−0.740), a 2.91-fold lower geometric-mean IC50. All 14 leave-one-out iterations retain p<0.05, and lineage contrasts preserve the response direction.](FIGURES/fig3_stress_tests.png){#fig:stress}

![**MBD4 genotype is decoupled from the PARP1-high trapping state.** PARP1 expression does not increase in MBD4-LOF models (median 6.77 vs 6.66 log1p TPM; p=0.605; n=19 vs 1,498), while baseline PARP1 expression predicts PARP inhibitor response across 481 matched lines (Spearman ρ=−0.416; p=1.36×10⁻²¹). The combined result supports expression-gated PARPi evaluation rather than MBD4-only routing.](FIGURES/fig4_parp1_expression.png){#fig:parp1}

![**Dual-axis MBD4 therapeutic-routing architecture.** Cytidine analogs load MBD4-deficient replication forks with unresolved BER substrate; ATR inhibition removes checkpoint-mediated fork stabilization; WEE1 inhibition defines a class-concordant expansion arm. The convergent trial hypothesis targets replication-fork collapse while the PARP branch remains gated by independent PARP1-high or BRCA/HRD evidence.](FIGURES/Figure_4.png){#fig:mechanism}
